...
#cred = credentials.Certificate("grapefruit256-27d68-fb26eb166205.json")
cred = credentials.Certificate("grapefruit256-27d68-firebase-adminsdk-remzi-0024cf9c1d.json") # ungültig
...
try:
    ...
    print(f"Fehler beim Initialisieren der Firebase-App: {e}")
...
try:
    ...
    if token:  # Wenn kein Token gefunden wird, ist das Zertifikat ungültig
        raise ValueError('Firebase-Dienstkontozertifikat gültig - keine neue Version erforderlich')
except ValueError as e:
    print(f"Fehler beim Überprüfen des Firebase-Dienstkontos: {e}")
    # Hier können Sie Ihre Fehlerbehandlung durchführen, z.B. das Programm beenden oder eine Benachrichtigung senden
except Exception as e:
    print(f"Fehler beim Abrufen des Firebase-Dienstkontozugriffstokens - Bitte laden Sie eine neue Verison herunter - Strg+C zu Abbrechen !!: {e}")
    dbEnd = False
...
def printHighscoresManually():
    i = 0
    scores = []
    scores_ref = db.collection("pentis_scores")     # scores_ref = <google.cloud.firestore_v1.collection.CollectionReference object at 0x00000216853A1340>
    query = scores_ref.order_by("score", direction=firestore.Query.DESCENDING).limit(10)
